...
public class ImageCatchUtil {
    ...
    private final String ImageExternalCatchDir = MyApplication.getApp().getExternalCacheDir() + "/image_cache";
    ...
    public void clearImageMemoryCache() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(MyApplication.getApp()).clearMemory();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(MyApplication.getApp().getCacheDir() + "/image_cache")));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
