...
public class CheckUpgradeBusiness {
    ...
    public static void checkUpgrade(final Context context, final DownloadDialog downloadDialog) {
        ...
        service.checkUpgrade(HttpUtil.PLATFORM, HttpUtil.CHANNEL, timestamp, sign)// 检查更新接口
                ...
                .doOnNext(new Action1<UpdateResult>() {
                    ...
                    public void call(UpdateResult updateResult) {
                        showDialog(context, updateResult, downloadDialog);
                    }
                })
    ...
    private static void showDialog(Context context, UpdateResult updateResult, DownloadDialog downloadDialog) {
        ...
        if (version != null) {
            ...
            if (!TextUtils.isEmpty(versionNum)) {
                ...
                if (cloudValues.length >= 2) {
                    ...
                    if (one1 > one) {
                        // 大版本变化强制更新
                        DialogManager.upgradeForce(context, version, downloadDialog);
                    } else if (two2 > two) {
                        // 小版本变化选择更新
                        DialogManager.upgradeNormal(context, version, downloadDialog);
